...
@mcp.tool()
def list_tasks(
    assignee: Optional[str] = None, process_definition_key: Optional[str] = None
) -> str:
    """
    ...
    try:
        ...
        tasks = camunda_client.get_tasks(
            assignee=assignee, process_definition_key=process_definition_key
        )
...
@mcp.tool()
def complete_task(task_id: str, variables: Optional[Dict[str, Any]] = None) -> str:
    """
...
def list_process_instances(
    process_definition_key: Optional[str] = None, business_key: Optional[str] = None
) -> str:
    """
    ...
    try:
        ...
        instances = camunda_client.get_process_instances(
            process_definition_key=process_definition_key, **filters
        )
        ...
        return f"Found {len(instances)} process instance(s):\n\n" + "\n\n---\n\n".join(
            instance_list
        )
...
def start_process_instance(
    ...
    Args:
        ...
        variables: Optional variables to set when starting the process

    Returns:
        Details of the started process instance
    """
    try:
        logger.info(f"Starting process instance for: {process_definition_key}")

        instance_data = camunda_client.start_process_instance(
            process_definition_key=process_definition_key,
            business_key=business_key,
            variables=variables,
        )

        result_text = "Process instance started successfully!\n\n"
        result_text += f"Instance ID: {instance_data.get('id', 'Unknown')}\n"
        result_text += (
            f"Definition ID: {instance_data.get('definitionId', 'Unknown')}\n"
        )
        result_text += f"Business Key: {instance_data.get('businessKey', 'None')}\n"
        result_text += f"Process Definition Key: {process_definition_key}\n"
        result_text += (
            f"Status: {'Ended' if instance_data.get('ended') else 'Active'}\n"
        )
        result_text += f"Suspended: {'Yes' if instance_data.get('suspended') else 'No'}"

        if instance_data.get("tenantId"):
            result_text += f"\nTenant: {instance_data['tenantId']}"
        ...
        return f"Error starting process instance: {str(e)}"
