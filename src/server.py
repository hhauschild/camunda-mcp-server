...
def start_process(
    ...
    """
    Start a new Camunda process instance.
    ...
    """
    try:
        logger.info(f"Starting process: {process_definition_key}")

        instance = camunda_client.start_process(
        ...
        result_text += f"Instance ID: {instance.id}\n"
        result_text += f"Definition ID: {instance.definition_id}\n"
        result_text += f"Business Key: {instance.business_key or 'None'}\n"
        result_text += f"Status: {'Ended' if instance.ended else 'Active'}\n"
        result_text += f"Suspended: {'Yes' if instance.suspended else 'No'}"
