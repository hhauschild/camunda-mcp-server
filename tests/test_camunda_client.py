...
class TestCamundaClient:
    ...
    @patch("src.camunda.client.requests.Session.request")
    def test_start_process_instance_success(self, mock_request: Mock) -> None:
        """Test successful process instance creation."""
        mock_response = Mock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {
            "id": "proc-instance-123",
            "definitionId": "test-process:1:def-456",
        ...
        with camunda_test_environment() as config:
            ...
            result = client.start_process_instance(
                process_definition_key="test-process",
                business_key="business-key-123",
                variables={"amount": 1000},
            )

        assert result["id"] == "proc-instance-123"
        assert result["definitionId"] == "test-process:1:def-456"
        assert result["businessKey"] == "business-key-123"
        ...
        assert args[0] == "POST"
        assert "process-definition/key/test-process/start" in args[1]
        assert "json" in kwargs

        # Check payload structure
        payload = kwargs["json"]
        assert payload["businessKey"] == "business-key-123"
        assert "variables" in payload
        assert payload["variables"]["amount"]["value"] == 1000
